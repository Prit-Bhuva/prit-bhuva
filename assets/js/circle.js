...
function drawPoints() {
    ...
    points.forEach((point) => {
        if (target.x && target.y) {
            ...
            point.circle.opacity = distance < 4000 ? 0.4 : distance < 20000 ? 0.3 : distance < 40000 ? 0.2 : 0.1;  
        }
        drawLines(point);
        point.circle.draw();
    });
...
function drawLines(p) {
    if (p.opacity > 0) {
        ctx.strokeStyle = `rgba(255,255,255,${p.opacity * 0.6})`; // Reduced brightness
...
function Circle(pos, rad, color) {
    ...
    this.opacity = 0.2; // Lowered opacity for circles

    this.draw = function () {
        if (this.opacity > 0) {
            ctx.beginPath();
            ctx.arc(this.pos.x, this.pos.y, this.radius, 0, Math.PI * 2, false);
            ctx.fillStyle = `rgba(255,255,255,${this.opacity * 1.0})`; // Adjusted brightness
            ctx.fill();
        }
    };
}
